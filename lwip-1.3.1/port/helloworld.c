...
/**
  ...
  * @retval error value
  */
err_t HelloWorld_recv(void *arg, struct tcp_pcb *pcb, struct pbuf *p, err_t err)
{
  struct pbuf *q;
...
/*void HelloWorld_Client_init(void)
{
	struct tcp_pcb *tcp_client_pcb=NULL;
	struct ip_addr ipaddr;
	IP4_ADDR(&ipaddr, 172, 16, 0, 99);
	tcp_client_pcb=tcp_new();
	tcp_connect(tcp_client_pcb,&ipaddr,8765,);
} */
