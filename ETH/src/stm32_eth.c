  ...
  {  
    ...
    if((RegValue & PHY_Duplex_Status) != (uint32_t)RESET)
    {
      ...
      ETH_InitStruct->ETH_Mode = ETH_Mode_FullDuplex;
			//My_Error((uint8_t *)"ETH Mode is Full-Duplex");
    ...
    else
    {
      ...
      ETH_InitStruct->ETH_Mode = ETH_Mode_HalfDuplex;  
			//My_Error((uint8_t *)"ETH Mode is Half-Duplex");
    }
    /* Configure the MAC with the speed fixed by the autonegotiation process */
    if(RegValue & PHY_Speed_Status)
    {  
      /* Set Ethernet speed to 10M following the autonegotiation */    
      ETH_InitStruct->ETH_Speed = ETH_Speed_100M;    //add ETH_Speed_100M
			//My_Error((uint8_t *)"ETH Speeed is 10M");
    }
    else
    {   
      /* Set Ethernet speed to 100M following the autonegotiation */ 
      ETH_InitStruct->ETH_Speed = ETH_Speed_100M;
			//My_Error((uint8_t *)"ETH Speeed is 100M");      
    }    
  }
  else
  {
...
void ETH_StructInit(ETH_InitTypeDef* ETH_InitStruct)
{
  ...
  ETH_InitStruct->ETH_Speed = ETH_Speed_100M;           //----------------add ETH-seppd 10M            
